...
contextBridge.exposeInMainWorld("vnusAgent", {
  ...
  onWaitingForPlan:     (cb) => ipcRenderer.on("waiting-for-plan",     (_, d) => cb(d)),
  onPlanVerified:       (cb) => ipcRenderer.on("plan-verified",        (_, d) => cb(d)),
  ...
  retryModelDownload:   ()    => ipcRenderer.send("retry-model-download"),
});